...
module.exports = {
    ...
    difficulty(path, options = []) {
        return new Promise((resolve, reject) => {
            const chunks = [];
            const calculator = spawn('dotnet', [`${path}/PerformanceCalculator.dll`, 'difficulty', path].concat(options));
    ...
    performance(path, options = []) {
        return new Promise((resolve, reject) => {
            const chunks = [];
            const calculator = spawn('dotnet', [`${path}/PerformanceCalculator.dll`, 'performance', path].concat(options));
            calculator.stdout.on('data', (data) => { chunks.push(data.toString()); });
            calculator.stderr.on('data', (data) => reject(data.toString()));
            calculator.on('exit', (code) => {
    ...
    profile(apiKey, user, options = []) {
        return new Promise((resolve, reject) => {
            const chunks = [];
            const calculator = spawn('dotnet', [`${path}/PerformanceCalculator.dll`, 'profile', user, apiKey].concat(options));
            calculator.stdout.on('data', (data) => { chunks.push(data.toString()); });
            calculator.stderr.on('data', (data) => reject(data.toString()));
            calculator.on('exit', (code) => {
                const data = chunks.join('');
